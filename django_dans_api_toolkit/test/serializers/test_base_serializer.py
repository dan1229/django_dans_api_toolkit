...
from .models import SampleSerializer, SampleModel


class BaseSerializerTestCase(TestCase):

    def setUp(self):
        self.instance = SampleModel(field1="value1", field2="value2", field3="value3")

    def test_default_serialization(self):
        serializer = SampleSerializer(self.instance)
        data = serializer.data
        self.assertEqual(set(data.keys()), {"field1", "field2", "field3"})

    def test_masked_serialization(self):
        serializer = SampleSerializer(self.instance, masked=True)
        data = serializer.data
        self.assertEqual(set(data.keys()), {"field1", "field2"})
